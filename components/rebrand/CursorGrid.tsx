...
type FalloffCurve = (t: number) => number;

const FALLOFF_CURVES: Record<string, FalloffCurve> = {
  linear: (t) => t,
  smooth: (t) => t * t * (3 - 2 * t),
  sharp: (t) => t * t * t,
};
...
// Props once resolved with their defaults — every field is guaranteed present,
// so the draw loop can read them without null assertions.
type ResolvedProps = Required<Omit<CursorGridProps, "className">>;
...
const CursorGrid = ({
  ...
  const propsRef = useRef<ResolvedProps>({} as ResolvedProps);
  const wakeRef = useRef<(() => void) | null>(null);

  // Keep the latest resolved props available to the long-lived effect without
  // re-subscribing event listeners on every prop change. Done in an effect so
  // we never mutate a ref during render.
  useEffect(() => {
    propsRef.current = {
      cellSize,
      color,
  ...
  useEffect(() => {
    ...
    // Only react while the pointer is over the hero (plus a radius margin), so
    // moving the mouse elsewhere on the page doesn't keep the canvas alive.
    const withinBounds = (e: PointerEvent): boolean => {
      const rect = rectOf();
      const m = propsRef.current.radius;
      return (
    ...
    // Listen on window so the effect still tracks the cursor when it passes
    // over the hero's text and interactive elements (the canvas itself stays
    // non-interactive and behind the content).
