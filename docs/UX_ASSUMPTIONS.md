...
Every wireframe frame carries `confirmed`, `working` or `content-dependent` annotations. When an open fact is resolved, update `DECISIONS.md`, the relevant spec and the frame together. A content-dependent block may be removed or resized, but it may not be silently filled with invented copy or proof.
