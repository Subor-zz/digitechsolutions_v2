...
| D-025 | De huidige documentatiemap bevat geen auditeerbare applicatie of Git-historie. Voorkeur: een schone applicatie in de bevestigde canonieke repository, met bronherstel als eerste technische gate. | LOCKED |
...
De inhoudelijke basisdeliverables en onderzoeksroutes staan in `MODERNIZATION_SCAN.md`. Commerciële omvang, duur en prijs blijven open.
...
| O-007 | Voorlopige classificatie staat in `CONTENT_AND_REDIRECT_AUDIT.md`; definitieve mapping wacht op Search Console/analytics/backlinks. | SEO/technical owner — before implementation |
...
| O-012 | Welke GitHub-repository en Vercel-project zijn canoniek en wie bezit deploy/DNS/rollback? | Subor/technical owner — before repository initialization |
| O-013 | Wordt `www` of apex de canonieke host? | Technical/SEO owner — before redirect implementation |
| O-014 | Welke exacte framework-, test- en motionversies worden vastgelegd na audit van de production source? | Technical owner — engineering-foundation sprint |
