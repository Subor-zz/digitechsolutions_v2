...
Build a **clean Next.js application inside the confirmed canonical repository**. The current documentation snapshot is not a Git repository and the live source is unavailable, so the stack below remains a reviewed foundation rather than an implemented decision. Recover and inspect the production repository before initialization; if it cannot be recovered, formally designate and initialize this folder as the new source of truth.
...
No dependency is added by this document. Package selection and versions are locked only in the engineering-foundation sprint after source recovery.
...
Prefer a privacy-minimal product that supports cookieless aggregate measurement. The legal basis and whether consent is required must be confirmed with the actual configuration, not vendor marketing. Centralize the required typed events from `REQUIREMENTS.md`; prohibit email, names, organization, free text, full query strings and stable user IDs in event properties.
...
1. Recover production repository/Vercel access and record versions, environment keys, DNS and rollback owner.
2. Export Search Console/analytics and crawl all current URLs, titles, canonicals, status codes and backlinks.
3. Initialize the clean app on an isolated branch; keep the live deployment unchanged.
...
Canonical repository and Vercel ownership; exact framework/tool versions; form provider and retention; analytics/consent product and legal basis; canonical host; error monitoring; scan commercial facts; and whether a scheduling tool is necessary. Owners and target gates are recorded in `DECISIONS.md` and the production plan.
